...
- **P-frame** — chỉ lưu **phần khác biệt** so với frame trước.
...
là một NAL độc lập. Mất một gói là hỏng một *dải ngang*, không phải cả khung hình.
Ảnh trong [`packet-loss.md`](../video/results/packet-loss.md) cho thấy đúng các
dải đó.
...
Cơ chế, với GOP 30 frame (I-frame ở #0 và #30):

1. Frame #5 mất một slice → hình vỡ ở dải đó.
...
frame 30     : mean abs diff =  0,3   ← IDR
frame 60     : mean abs diff =  0,0   ← IDR sạch, trùng khít từng bit
```

Lý do không phai: P-frame chép nguyên khối sai từ frame trước. Chép đúng cái sai.

**Nhưng "mất 1 gói hỏng nửa GOP" là sai.** Gói mất phân bố đều theo *gói*, không
phải theo *frame*, mà keyframe chiếm **15% số gói** của luồng (nó to gấp 15 lần
P-frame). Trúng một gói của keyframe là mất trọn 30 frame. Tính theo phân bố gói
...
**Và một hệ quả khó chịu:** decoder **không báo** cho ứng dụng biết. Ở 20% mất gói,
`receiver.cpp` vẫn nhận 148/150 frame, `GST_BUFFER_FLAG_DISCONT` = 0,
`GST_BUFFER_FLAG_CORRUPTED` = 0. Muốn biết link đang xấu thì phải đếm ở **tầng
RTP** (`rtpjitterbuffer` property `stats` → `num-lost`), chứ không có gì phía sau
decoder nói cho bạn.
...
> **Cái giá của `latency=0`, đo được.** Cho mạng jitter ±5 ms mà **không mất gói
> nào** (`netem delay 20ms 5ms`, `loss 0%`): kernel không đánh rơi gói nào, nhưng
> receiver chỉ giải mã được **67/300 frame**. 3495 gói đến nơi đầy đủ và bị chính
> jitter buffer vứt vì "đến muộn". Bật `latency=200` lên: **270–300/300 frame**.
> Nói cách khác, 200 ms độ trễ mua lấy khoảng cách giữa 67 và 300 frame.
> `latency=0` không chịu nổi một *mạng*; nó chỉ chịu nổi một *sợi dây*.
...
Nhiều bài viết gọi đây là "một dòng cho hiệu quả lớn nhất". Em đo thì **không**:
nó chỉ bớt **15 ms** trong tổng 2280 ms tiết kiệm được — encoder gánh 2064 ms.
Nhưng vẫn nên làm, và vì một lý do bất ngờ: chỗ nó *hiện ra* không phải chỗ nó
*gây hại*. Sink chờ PTS thì buffer dồn ứ ngược lên **jitter buffer** (15,5 ms →
0,8 ms khi tắt sync). Element báo triệu chứng không phải element có bệnh.
...
Cơ chế (đúng như `src/receiver.cpp` làm):
1. `gst_app_sink_set_callbacks()` gắn thẳng hàm `new_sample` vào appsink.
   Không cần `emit-signals=true` — cái đó dành cho đường signal của GObject,
   chậm hơn và chỉ cần khi bind từ ngôn ngữ khác.
2. Trong callback: `gst_app_sink_pull_sample()` → `GstSample` → `GstBuffer`.
3. `gst_buffer_map()` để lấy con trỏ vào pixel data. **Bắt buộc `gst_buffer_unmap()`** sau đó.
4. `GST_BUFFER_PTS(buf)` cho timestamp.
5. `gst_sample_unref()` — nếu quên, **memory leak**, RAM tăng dần cho tới khi bị kill.

Callback chạy trên **streaming thread**, không phải main loop. Nên các bộ đếm
trong `receiver.cpp` là `std::atomic`, còn `GMainLoop` chỉ đọc chúng mỗi giây.
...
Chạy trên `lo` (loopback) thì ảnh hưởng **toàn bộ** traffic loopback của máy — kể cả thứ khác. Nhớ `del` sau khi đo. Trong WSL, `tc` chỉ chạy được với `wsl -d Ubuntu-22.04 -u root`.

**`tc` không chỉ để bật lỗi — nó còn là dụng cụ đo.** `tc -s qdisc show dev lo` in
ra số gói đã qua và số gói bị đánh rơi. Đó là **sự thật của kernel**, và là thứ
duy nhất để đối chiếu khi một bộ đếm khác nói dối:

```bash
tc -s qdisc show dev lo
#  Sent 5398886 bytes 9490 pkt (dropped 20, overlimits 0 requeues 0)
```

Bài học từ T1.5: `num-lost` của `rtpjitterbuffer` **trùng khít** với `tc` khi mạng
chỉ mất gói (101 = 101), nhưng **vọt lên 3640** khi mạng đảo thứ tự gói — nó đếm
*số lần phát hiện khoảng trống*, không phải *số gói không tới nơi*. Một bộ đếm
chưa được đối chiếu với bộ đếm độc lập thì chưa phải số liệu.

Lưu ý: `netem loss X%` sinh lỗi **độc lập từng gói**. Sóng vô tuyến thật mất gói
theo **chùm**, và mất 20 gói liên tiếp khác hẳn mất 20 gói rải rác. Đây là giới
hạn của thí nghiệm này, phải nói thẳng nếu bị hỏi.
